...
import { InterviewState, Candidate, ChatMessage, Answer, Question } from '../types';
...
const interviewSlice = createSlice({
  ...
  initialState,
  reducers: {
    createCandidate: (state, action: PayloadAction<{ name?: string; email?: string; phone?: string; resumeFile?: File; resumeText?: string }>) => {
    ...
    submitAnswer: (state, action: PayloadAction<{ answer: string; timeSpent: number }>) => {
      if (state.currentCandidate && state.questions.length > 0) {
